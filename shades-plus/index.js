...
function Settings() {
  const { React, RN } = uiModules();
  const [, force] = React.useReducer(value => ~value, 0);
  ...
  const users = Object.values(getPath("users", {}));
  const created = snowflakeDate(userId);

  const editUser = record => {
    setUserId(record.id || "");
    setUsername(record.username || "");
    setLocalName(record.localName || "");
    setNote(record.note || "");
    setStatus("editando usuário salvo ♡");
  };

  const saveUser = () => {
    if (!/^\d{15,22}$/.test(String(userId).trim())) {
      setStatus("coloque um id de usuário válido");
      return;
    }
    setRecord(userId.trim(), { username, localName, note });
    setStatus("salvo com carinho :3");
    forceUpdate();
  };

  const backup = () => {
    const payload = JSON.stringify({ version: 1, users: getPath("users", {}) });
    setStatus(copyText(payload) ? "backup copiado ♡" : "não consegui acessar a área de transferência");
  };

  const importBackup = async () => {
    try {
      const clipboard = getClipboard();
      if (!clipboard?.getString) throw new Error("clipboard unavailable");
      const value = await clipboard.getString();
      const parsed = JSON.parse(value);
      if (!parsed?.users || typeof parsed.users !== "object") throw new Error("invalid");
      setPath("users", parsed.users);
      setStatus("backup importado ♡");
      forceUpdate();
    } catch (_) {
      setStatus("backup inválido ou área de transferência indisponível");
    }
  };
  ...
  const group = (title, children) =>
    React.createElement(
      RN.View,
      { style: styles.group, key: title },
      React.createElement(RN.Text, { style: styles.groupTitle }, title),
      children
    );
  ...
  return React.createElement(
    ...
    React.createElement(
      RN.View,
      { style: styles.hero },
      React.createElement(RN.Text, { style: styles.heroTitle }, "shades+ ♡"),
      React.createElement(
        RN.Text,
        { style: styles.heroSubtitle },
        "nomes mais claros, perfis úteis e detalhes locais — usando a mesma paleta do shades of revenge."
      ),
      React.createElement(RN.Text, { style: styles.badge }, "feito por lucky")
    ),

    group("perfil", [
      React.createElement(SwitchRow, {
        key: "p-user",
        label: "mostrar @ depois do nick",
        subLabel: "exemplo: lucky  @lucky",
        path: "profile.showUsername",
        icon: "AtIcon",
        forceUpdate
      }),
      React.createElement(SwitchRow, {
        key: "p-local",
        label: "usar apelidos locais",
        subLabel: "o apelido só aparece no seu Discord",
        path: "profile.useLocalNames",
        icon: "EditIcon",
        forceUpdate
      }),
      React.createElement(SwitchRow, {
        key: "p-note",
        label: "mostrar notas locais",
        subLabel: "a nota aparece discretamente no perfil",
        path: "profile.showNotes",
        icon: "BookIcon",
        forceUpdate
      }),
      React.createElement(SwitchRow, {
        key: "p-created",
        label: "mostrar criação da conta",
        subLabel: "calculada localmente pelo ID do Discord",
        path: "profile.showCreatedAt",
        icon: "CalendarIcon",
        forceUpdate
      }),
      React.createElement(SwitchRow, {
        key: "p-actions",
        label: "ações rápidas",
        subLabel: "copiar id e abrir avatar/banner em HD",
        path: "profile.showQuickActions",
        icon: "CopyIcon",
        forceUpdate
      })
    ]),
    ...
    group("chat", [
      React.createElement(SwitchRow, {
        key: "c-user",
        label: "mostrar @ depois do nick",
        subLabel: "adiciona o username ao cabeçalho das mensagens",
        path: "chat.showUsername",
        icon: "AtIcon",
        forceUpdate
      }),
      React.createElement(SwitchRow, {
        key: "c-local",
        label: "apelidos locais no chat",
        subLabel: "usa os nomes salvos abaixo",
        path: "chat.useLocalNames",
        icon: "ChatIcon",
        forceUpdate
      }),
      React.createElement(SwitchRow, {
        key: "c-time",
        label: "data e hora completas",
        subLabel: "mostra dd/mm/aaaa e horário quando compatível",
        path: "chat.fullTimestamp",
        icon: "ClockIcon",
        forceUpdate
      }),
      React.createElement(SwitchRow, {
        key: "c-edit",
        label: "editado compacto",
        subLabel: "troca o texto grande por “editado”",
        path: "chat.compactEdited",
        icon: "PencilIcon",
        forceUpdate
      }),
      React.createElement(SwitchRow, {
        key: "c-sep",
        label: "separadores suaves",
        subLabel: "linha roxa fina entre mensagens",
        path: "chat.separators",
        icon: "DividerIcon",
        forceUpdate
      })
    ]),
    ...
    group("aparência", [
      React.createElement(SwitchRow, {
        key: "a-soft",
        label: "@ em lilás suave",
        subLabel: "usa #a599c6 em vez do roxo mais forte",
        path: "appearance.softAccent",
        icon: "PaletteIcon",
        forceUpdate
      }),
      React.createElement(SwitchRow, {
        key: "a-compact",
        label: "@ menor e discreto",
        subLabel: "mantém o nick como informação principal",
        path: "appearance.compactSuffix",
        icon: "TextIcon",
        forceUpdate
      })
    ]),

    group("apelidos e notas locais", [
      ...
      React.createElement(RN.TextInput, {
        ...
        placeholder: "username sem @ (opcional)",
        placeholderTextColor: COLORS.muted,
        autoCapitalize: "none"
      }),
      React.createElement(RN.TextInput, {
      ...
      React.createElement(RN.TextInput, {
        ...
        placeholder: "nota local, tipo “amigo do gamedev”",
        placeholderTextColor: COLORS.muted,
        multiline: true,
        maxLength: 180
      }),
      created
        ? React.createElement(
            RN.Text,
            { key: "created-preview", style: styles.diagnostic },
            `essa conta foi criada em ${formatDate(created, false)}`
          )
        : null,
      React.createElement(
        RN.View,
        ...
        React.createElement(
          RN.Pressable,
          { style: [styles.button, styles.primaryButton], onPress: saveUser },
          React.createElement(RN.Text, { style: styles.buttonText }, "salvar")
        ),
        React.createElement(
          RN.Pressable,
          {
          ...
          React.createElement(RN.Text, { style: styles.buttonText }, "limpar campos")
        )
      ),
      status ? React.createElement(RN.Text, { key: "status", style: styles.status }, status) : null,
      ...users.map(record =>
        React.createElement(
          RN.View,
          { key: record.id, style: styles.savedCard },
          React.createElement(
            RN.Text,
            { style: styles.savedTitle },
            record.localName || record.username || record.id
          ),
          React.createElement(
            RN.Text,
            { style: styles.savedMeta },
            `${record.username ? `@${record.username}  •  ` : ""}${record.id}`
          ),
          record.note
            ? React.createElement(RN.Text, { style: styles.savedNote }, `♡ ${record.note}`)
            : null,
          React.createElement(
            RN.View,
            { style: styles.smallActions },
            React.createElement(
              RN.Pressable,
              { style: styles.smallButton, onPress: () => editUser(record) },
              React.createElement(RN.Text, { style: styles.smallButtonText }, "editar")
            ),
            React.createElement(
              RN.Pressable,
              {
              ...
              React.createElement(RN.Text, { style: styles.smallButtonText }, "copiar id")
            ),
            React.createElement(
              RN.Pressable,
              {
                style: [styles.smallButton, { backgroundColor: `${COLORS.red}22` }],
                onPress: () => {
              ...
              React.createElement(RN.Text, { style: [styles.smallButtonText, { color: COLORS.red }] }, "remover")
            )
          )
        )
      ),
      React.createElement(
        RN.View,
        { key: "backup-actions", style: styles.buttonRow },
        React.createElement(
          RN.Pressable,
          { style: [styles.button, styles.secondaryButton], onPress: backup },
          React.createElement(RN.Text, { style: styles.buttonText }, "copiar backup")
        ),
        React.createElement(
          RN.Pressable,
          { style: [styles.button, styles.secondaryButton], onPress: importBackup },
          React.createElement(RN.Text, { style: styles.buttonText }, "importar backup")
        )
      )
    ]),
    ...
    group("diagnóstico", [
      React.createElement(
        RN.Text,
        { key: "diag", style: styles.diagnostic },
        `patches ativos: ${diagnostics.active.length}\n` +
          `tentativas: ${diagnostics.attempted.length}\n` +
          (diagnostics.active.length
            ? diagnostics.active.join(", ")
            : "recarregue o Discord após mudar opções. alguns componentes dependem da versão do app.")
      )
    ])
  );
}
...
exports.storage = plugin.storage;
Object.defineProperty(exports, "__esModule", { value: true });
return exports;})({},bunny.api,bunny.metro,bunny.metro.common,vendetta.plugin,bunny.utils.lazy);